...
function renderDOM (year, month) {
  ...
  days.forEach((cellText) => {
    ...
    if (selectedMonth==currMonth && selectedYear == currYear && cellText == currDay) {
        cellNode.className = cellNode.className + " today";
    }
    cellNode.append(cellText);
    newCalendarNode.append(cellNode);
  });
...
function goToNextMonth () {
    selectedMonth++;
    if (selectedMonth === 13) {
        /* Fix a bug when start a new year and */
        selectedMonth = 1;
        selectedYear++;
    }
    selectedMonthDays = getDayCount(selectedYear, selectedMonth);
